...
class Buzzer:
    def __init__(self):
        ...
        self.beep_on = float(
            getattr(buzzer_cfg, "dose_ready_beep_seconds", 0.35)
            if buzzer_cfg
            else 0.35
        )
        self.beep_off = float(
            getattr(buzzer_cfg, "dose_ready_gap_seconds", 0.65)
            if buzzer_cfg
            else 0.65
        )
    ...
    def _alert_loop(self, duration_sec: float) -> None:
        logger.info("Dose-due buzzer started for %.0f seconds", duration_sec)
        ...
        try:
            while not self._stop.is_set() and time.monotonic() < deadline:
                ...
                if remaining <= 0 or self._stop.wait(min(self.beep_off, max(0.05, remaining))):
                    break
        finally:
            self._set(False)
            logger.info("Dose-due buzzer stopped")
    ...
    def alert_dose_due(self, duration_sec: float | None = None) -> None:
        ...
        # Keep within the requested 45–60 s window.
    ...
    def play(self, pattern_name: str, blocking: bool = False):
        """
        Legacy short-pattern API. Only dose_ready is supported, and it maps
        to the long dose-due alert (never used during dispensing).
        """
        if pattern_name != "dose_ready":
            logger.debug("Ignoring non dose-due buzzer pattern: %s", pattern_name)
            return
        if blocking:
            # Blocking long alerts would freeze Verify Now — always background.
            self.alert_dose_due()
            return
        self.alert_dose_due()
